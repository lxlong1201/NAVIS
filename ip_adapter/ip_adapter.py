...
class IPAdapter:
    ...
    def set_num_ref(self, num_ref):
        for attn_processor in self.pipe.unet.attn_processors.values():
            if isinstance(attn_processor, IPAttnProcessor):
                attn_processor.num_ref = num_ref
...
class StoryAdapterXL(IPAdapter):
    ...
    def generate(
        ...
        **kwargs,
        ...
        if use_image:
            if pil_image is not None:
                self.set_num_ref(len(pil_image))
                print("len", len(pil_image))
                num_prompts = 1
            else:
                num_prompts = clip_image_embeds.size(0)
        else:
        ...
        # prompt有添加内容
            ...
            if style == "comic":
                ...
                negative_prompt = "deformed, multiple limbs, extra arms, extra legs, poorly drawn hands, bad anatomy, mutated, disfigured, duplicate limbs"
                prompt = "best quality, high quality, cute illustration, cartoon style, soft lighting, " + prompt + ", soft shading, clean lines, rounded shapes, children's book art, highly detailed"
            else:
        ...
        with torch.inference_mode():
            ...
            print(f'prompt_embeds:{prompt_embeds_.shape}')
            if use_image:
                prompt_embeds = torch.cat([prompt_embeds_, image_prompt_embeds], dim=1)
                negative_prompt_embeds = torch.cat([negative_prompt_embeds_, uncond_image_prompt_embeds], dim=1)
            else:
